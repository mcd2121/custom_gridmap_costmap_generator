...
def generate_launch_description():
    # Find the custom_gridmap_costmap_generator package share directory
    custom_gridmap_costmap_generator_dir = get_package_share_directory('custom_gridmap_costmap_generator')
    ...
    declare_filters_config_file_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            custom_gridmap_costmap_generator_dir, 'config', 'mapping_multi_obs_config.yaml'),
    ...
    declare_visualization_config_file_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            custom_gridmap_costmap_generator_dir, 'config', 'mapping_viz.yaml'),
        description='Full path to the Gridmap visualization config file to use')

    declare_rviz_config_file_cmd = DeclareLaunchArgument(
        'rviz_config',
        default_value=os.path.join(
            custom_gridmap_costmap_generator_dir, 'rviz', 'display.rviz'),
        description='Full path to the RVIZ config file to use')

    # Declare node actions
    grid_map_filter_node = Node(
        package='custom_gridmap_costmap_generator',
    ...
    image_to_gridmap_node = Node(
        package='custom_gridmap_costmap_generator',
        executable='image_to_gridmap_node',
